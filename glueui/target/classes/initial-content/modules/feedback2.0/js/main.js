...
(function(){
	...
	*/
	...
	// 修改feedBody height 
	//按照todo来判断是增加or减少
	function modHeightOfFeedBody(todo){
		require(["dojo/query"],function(query){
			var feedBody=query(".feedBody")[0];
			console.log(query(".feedBody"));
			var height=getCurrentStyle(feedBody,"height");
			
			if(todo == "raise"){
				newHeight = parseInt(height.split('px')[0])+50;
			}else if(todo == "lessen"){
				newHeight = parseInt(height.split('px')[0])-50;
			}else{
				return;
			}
			query(".feedBody")[0].style.height = newHeight.toString()+"px";
			});	
	}

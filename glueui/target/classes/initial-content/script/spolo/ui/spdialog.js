...
(function(){
	...
	*/
	_spDialog = function(args)
	{
		...
		if(args["widthradio"] && args["heightradio"])
		{
			widthradio = args["widthradio"];			
			heightradio = args["heightradio"];
		}
		width = Math.ceil(Client.viewportWidth() * widthradio);
		height = Math.ceil(Client.viewportHeight() * heightradio);
		
		if(args["widthpx"] && args["heightpx"])
		{
			width = args["widthpx"];			// widthpx : 窗口像素宽度,     // 默认为800
			height = args["heightpx"];			// heightpx : 窗口像素高度,    // 默认为600
			_spDialogResize = false;
		}		
...
//================================================================================================	
	...
	_spDialogEmit = function(id, eventType, param){		// spdWindow : SanPolo dialog window
		...
		if(!spdialogMap[id]["handle"]){
			console.error("如果要使用emit方法处理事件，new spdialog 时必须传入 callback ");
			return;
		}

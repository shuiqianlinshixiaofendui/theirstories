...
require(
	...
	function(on, ready, lang, query, Dialog, Button, domConstruct, waterfall, search, AdvCategory, Toolbar, folder_modellib, folder_cls, user_cls,Mask){
		...
		ready(function(){
			...
			({
				...
				success:function(cateData)
				{
					...
					//console.log(query(".clickEnv"));
					
					on(query(".clickEnv"), "click", function(){
						
						
						var judge = document.getElementById("testmy").value;
						if(judge == "关闭")
						{
							document.getElementById("testmy").value = "展开";
							document.getElementById("catgory").style.display = "none";
						
						}else{
							document.getElementById("testmy").value = "关闭";
							document.getElementById("catgory").style.display = "block";
			...
			conditions["callback"] = function(initDataList, totalNum){
				...
				on(waterfall_ref,"sourceDownload",function(args){
                    ...
                    pathArray.push(args);
                    pathArray.push(1);
                    scene.addItemsByCount(pathArray,function(res)
					{
						Mask.hide();
		...
		function doSearch(filterData, waterfall_ref){
			...
			getModellist2.call(this, conditions);
		}
		...
		function advCategoryCategoryItemChange(json, waterfall_ref){
			...
			for(var i in json){
				switch(i)
				{
					case "/content/roomlibcategory/room":
						categoryString["roomPath"] = json[i][json[i].length-1];
						break;
                    case "/content/roomlibcategory/area":
						categoryString["areaPath"] = json[i][json[i].length-1];
						break;
					default:
						break;
				}
		...
		function analyseJSON(data){
			...
			for(var index in data["data"]){
				var date = setpublishdate.call(this, data["data"][index]["jcr:created"]);
				var publishAuthor = data["data"][index]["jcr:createdBy"];
				...
				var previewurl = data["data"][index]["preview"];

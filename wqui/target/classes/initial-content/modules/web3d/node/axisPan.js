...
define("web3d/node/axisPan",["dojo/topic"],function(topic){
// 缓冲被点击模型
	...
	return dojo.declare([],{
		constructor : function(x3d){
			...
			// 接收消息，将坐标系改变成直线 x方向
			topic.subscribe("coorDrag/coor_x", function(model){
				moveLine("x",model);
			});
			
			// 接收消息，将坐标系改变成直线 y方向
			topic.subscribe("coorDrag/coor_y", function(model){
				moveLine("y",model);
			});
			
			// 接收消息，将坐标系改变成直线 z方向
			topic.subscribe("coorDrag/coor_z", function(model){
				moveLine("z",model);
			});
			
			// 接收消息，当鼠标抬起时 
			topic.subscribe("coorDrag/mouseUp", function(){

...
define("web3d/node_new/axisTranslate",["dojo/topic"],function(topic){
// 缓冲被点击模型
	...
	return dojo.declare([],{
		constructor : function(x3d){
			...
			// 接收消息，将坐标系改变成直线 x方向
			this.coorDrag_coorxHandle = topic.subscribe("coorDrag/coor_x1", function(model){
				moveLine("x",model);
			});
			
			// 接收消息，将坐标系改变成直线 y方向
			this.coorDrag_cooryHandle = topic.subscribe("coorDrag/coor_y1", function(model){
				moveLine("y",model);
			});
			
			// 接收消息，将坐标系改变成直线 z方向
			this.coorDrag_coorzHandle = topic.subscribe("coorDrag/coor_z1", function(model){
				moveLine("z",model);
			});
			
			// 接收消息，当鼠标抬起时 
			this.coorDrag_mouseUpHandle = topic.subscribe("coorDrag/mouseUp1", function(){

...
define("web3d/operate/operate_coordinateRotation",["dojo/topic","dojo/_base/lang"],function(topic,lang){
return dojo.declare([],{
		constructor : function(trans){
			...
			this.onMouseReleaseHandle = topic.subscribe("system/onMouseRelease", function(x,y,buttonState){
				if(mouseState && Spolo.modelOperationMode==2 && Spolo.OperationMode == 2){
					...
					selectModel = null;
					// 当鼠标抬起时		
					topic.publish("coorDrag/mouseUp_Rotation1");
					/**
					 * 获取模型数据
					 */
			...
			function moveOnCoordinate(x,y,shape,theMoveObject){
				if(shape && theMoveObject){
					...
					// 计算当前点到物体中心的向量
					var point = cpos.subtract(pos);
                    // 计算点击点到物体中心的向量
                    var point2 = ipos.subtract(pos);
                    var rot = x3dom.fields.Quaternion.rotateFromTo(point2,point).toAxisAngle();
                    
					switch (id){
						
						case "mater_Rotation0" :
							// 当模型在坐标系下移动时，publish一个消息，坐标系module接收这个消息，改变坐标系
							topic.publish("coorDrag/coor_z_Rotation1",Spolo.selectedObj);
							//将旋转值赋予模型
							var sign = rot[0].x > 0 ? 1 : -1;
                            Spolo.selectedObj.localRotate(new x3dom.fields.SFVec3f(0,0,sign),rot[1]/2);
							//_xmlNode.setAttribute("rotation",0+" "+0+" "+1+" "+(mousePressrotation.w-rotX-rotY));
							
							break;
						case "mater_Rotation1" :
							// 当模型在坐标系下移动时，publish一个消息，坐标系module接收这个消息，改变坐标系
							topic.publish("coorDrag/coor_y_Rotation1",Spolo.selectedObj);
							//将旋转值赋予模型trans.localRotate(axis,rotX-rotY);
							var sign = rot[0].x > 0 ? 1 : -1;
                            Spolo.selectedObj.localRotate(new x3dom.fields.SFVec3f(sign,0,0),rot[1]/2);
							//Spolo.selectedObj._xmlNode.setAttribute("rotation",1+" "+0+" "+0+" "+(mousePressrotation.w-rotX-rotY));
							break;
						case "mater_Rotation2" : 
							
							// 当模型在坐标系下移动时，publish一个消息，坐标系module接收这个消息，改变坐标系
							topic.publish("coorDrag/coor_x_Rotation1",Spolo.selectedObj);
							//将旋转值赋予模型
							var sign = rot[0].x > 0 ? 1 : -1;
                            Spolo.selectedObj.localRotate(new x3dom.fields.SFVec3f(0,sign,0),rot[1]/2);
